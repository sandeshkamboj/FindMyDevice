...
class SurveillanceService : Service() {
    ...
    // Removed updateNotificationForCommand and updateNotificationForUpload, as their methods no longer exist
    // and you requested only a simple notification be shown at all times.
    ...
    private suspend fun startCommandListener() = coroutineScope {
        while (isActive) {
            ...
            delay(20000L)
            // Previously: handler.post { updateNotificationForCommand(command) }
            // Simulate handling the command for 5 seconds
            delay(5000L)
            // Previously: handler.post { revertNotification() }
            // Simulate file upload after command
            delay(2000L)
            // Previously: handler.post { updateNotificationForUpload() }
